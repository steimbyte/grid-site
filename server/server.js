...
class SettingsManager {
	...
	static #loadSettings() {
		if (!existsSync(SettingsManager.#settingsFile))
			return { uploadsEnabled: false, userSettings: {} };
		try {
			return JSON.parse(readFileSync(SettingsManager.#settingsFile, "utf-8"));
		} catch {
			return { uploadsEnabled: false, userSettings: {} };
		}
	}
	...
	static #saveSettings(data) {
		writeFileSync(SettingsManager.#settingsFile, JSON.stringify(data, null, 2));
	}

	// Global uploads setting
	static isUploadsEnabled() {
		return SettingsManager.#loadSettings().uploadsEnabled !== false;
	}
	...
	static updateSettings(updates) {
		const settings = { ...SettingsManager.#loadSettings(), ...updates };
		SettingsManager.#saveSettings(settings);
		return settings;
	}
}
...
// Ensure directories
[CONFIG.sitesDir, join(CONFIG.sitesDir, "thumbnails"), CONFIG.usersDir].forEach(
	(dir) => {
...
class UserManager {
	...
	static #loadData() {
		if (!existsSync(UserManager.#usersFile)) return { users: [] };
		try {
			return JSON.parse(readFileSync(UserManager.#usersFile, "utf-8"));
		} catch {
			return { users: [] };
		}
	}
	...
	static #saveData(data) {
		writeFileSync(UserManager.#usersFile, JSON.stringify(data, null, 2));
	}
	...
	static async createUser({ username, password, role = "user" }) {
		const data = UserManager.#loadData();
		...
		return UserManager.#sanitizeUser(user);
	}
	...
	static getUsers() {
		return UserManager.#loadData().users.map((u) =>
			UserManager.#sanitizeUser(u),
		);
	}
	static getUser(id) {
		const user = UserManager.#loadData().users.find((u) => u.id === id);
		return user ? UserManager.#sanitizeUser(user) : null;
	}
	...
	static async updateUser(id, updates, callerRole = "user") {
		...
		if (updates.password && callerRole !== "admin")
			throw new Error("Forbidden");
		if (updates.role && callerRole !== "admin") throw new Error("Forbidden");

		// Hash password if provided
		if (updates.password)
			updates.password = await bcrypt.hash(updates.password, CONFIG.saltRounds);
		...
		UserManager.#saveData(data);
		return UserManager.#sanitizeUser(data.users[idx]);
	}
...
class StatsManager {
	...
	static #loadStats() {
		if (!existsSync(StatsManager.#statsFile))
			return {
				totalLogins: 0,
				siteViews: {},
				createdAt: new Date().toISOString(),
		...
		try {
			return JSON.parse(readFileSync(StatsManager.#statsFile, "utf-8"));
		} catch {
			return {
				totalLogins: 0,
				siteViews: {},
	...
	static #saveStats(data) {
		writeFileSync(StatsManager.#statsFile, JSON.stringify(data, null, 2));
	}
	static recordLogin() {
		const s = StatsManager.#loadStats();
		s.totalLogins = (s.totalLogins || 0) + 1;
		StatsManager.#saveStats(s);
	}
	static recordSiteView(id) {
		const s = StatsManager.#loadStats();
		s.siteViews[id] = (s.siteViews[id] || 0) + 1;
		StatsManager.#saveStats(s);
	}
	...
	static getStats() {
		...
		return {
			...
			totalUsers: UserManager.getUsers().length,
			totalSites: readdirSync(CONFIG.sitesDir).filter((f) =>
				f.endsWith(".json"),
			).length,
			siteViews: s.siteViews || {},
		};
	}
}
...
class TagsManager {
	...
	static #loadTags() {
		if (!existsSync(TagsManager.#tagsFile)) return { tags: [], siteTags: {} };
		try {
			return JSON.parse(readFileSync(TagsManager.#tagsFile, "utf-8"));
		} catch {
			return { tags: [], siteTags: {} };
		}
	}

	static #saveTags(data) {
		writeFileSync(TagsManager.#tagsFile, JSON.stringify(data, null, 2));
	}

	static getTags() {
		return TagsManager.#loadTags().tags;
	}

	static addTag(name, color) {
		const data = TagsManager.#loadTags();
		if (data.tags.find((t) => t.name.toLowerCase() === name.toLowerCase()))
			throw new Error("Tag exists");
		data.tags.push({ id: randomUUID(), name, color });
		TagsManager.#saveTags(data);
		return data.tags;
	}
...
app.get("/api/admin/users/:id", authMiddleware(["admin"]), (req, res) => {
	const u = UserManager.getUser(req.params.id);
...
app.put("/api/admin/users/:id", authMiddleware(["admin"]), async (req, res) => {
	try {
		res.json(
			await UserManager.updateUser(req.params.id, req.body, req.user.role),
		);
	} catch (err) {
		res.status(400).json({ error: err.message });
	}
...
app.get("/api/leaderboard", authMiddleware(), (req, res) =>
	res.json(UserManager.getLeaderboard(parseInt(req.query.limit) || 10)),
);

// Tags
app.get("/api/tags", (req, res) => res.json(TagsManager.getTags()));
app.post("/api/tags", authMiddleware(["admin"]), (req, res) => {
	try {
		res.json(TagsManager.addTag(req.body.name, req.body.color));
	} catch (err) {
		res.status(400).json({ error: err.message });
	}
});
app.delete("/api/tags/:id", authMiddleware(["admin"]), (req, res) => {
	try {
		res.json(TagsManager.deleteTag(req.params.id));
	} catch (err) {
		res.status(400).json({ error: err.message });
	}
});
...
app.get("/api/sites", authMiddleware(), (req, res) => {
	try {
		...
		const sites = readdirSync(CONFIG.sitesDir)
			...
			.map((f) => {
				try {
					...
					return {
						...
						views: stats.siteViews[site.id] || 0,
						tags,
						icon: site.icon,
					};
				} catch {
	...
	try {
		...
		if (req.user.role !== "admin") {
			...
			if (!SettingsManager.canUserUpload(req.user.userId)) {
				return res
					.status(403)
					.json({ error: "Uploads are disabled for your account" });
			}
		}
		...
		const site = {
			...
			order: Date.now(),
			icon: req.body.icon?.replace(/[^a-z0-9-]/gi, "") || "file",
		};
		writeFileSync(
			join(CONFIG.sitesDir, `${id}.json`),
...
app.put("/api/sites/:id", authMiddleware(), (req, res) => {
	try {
		...
		if (!existsSync(filename))
			return res.status(404).json({ error: "Not found" });
		const site = JSON.parse(readFileSync(filename, "utf-8"));
		if (site.uploadedBy !== req.user.username && req.user.role !== "admin")
		...
		if (req.body.content)
			writeFileSync(join(CONFIG.sitesDir, `${site.id}.html`), req.body.content);
		if (req.body.tags !== undefined)
			TagsManager.setSiteTag(site.id, req.body.tags);
		res.json({ success: true });
	} catch (err) {
		res.status(500).json({ error: err.message });
	}
...
app.delete("/api/sites/:id", authMiddleware(["admin"]), (req, res) => {
	try {
		...
		[".json", ".html"].forEach((ext) => {
			const f = join(CONFIG.sitesDir, `${id}${ext}`);
...
app.get("/api/export", authMiddleware(), (req, res) => {
	try {
		const files = readdirSync(CONFIG.sitesDir).filter((f) =>
			f.endsWith(".html"),
		);
		...
		const content = files
			.map(
				(f) =>
					`=== ${f} ===\n${readFileSync(join(CONFIG.sitesDir, f), "utf-8")}`,
			)
			.join("\n\n");
		res.send(content);
	} catch (err) {
...
// Settings (admin only)
app.get("/api/settings", authMiddleware(["admin"]), (req, res) =>
...
async function start() {
	...
	app.listen(CONFIG.port, "0.0.0.0", () => {
		...
		console.log(
			`  Admin: ${process.env.ADMIN_USERNAME || "admin"} / ${process.env.ADMIN_PASSWORD || "admin"}\n`,
		);
	});
}
