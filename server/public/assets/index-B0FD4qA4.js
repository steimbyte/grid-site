var $=Object.defineProperty;var k=(m,e,t)=>e in m?$(m,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):m[e]=t;var b=(m,e,t)=>k(m,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function t(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(i){if(i.ep)return;i.ep=!0;const a=t(i);fetch(i.href,a)}})();class C{static render(){return`
    ...
    `}static init(e){const t=document.getElementById("login-username"),s=document.getElementById("login-password"),i=document.getElementById("btn-login"),a=i.querySelector(".btn-text"),o=document.getElementById("btn-toggle"),h=document.getElementById("toggle-text"),d=document.getElementById("login-error"),r=document.getElementById("login-card"),c=document.getElementById("card-glow");let u=!1;r==null||r.addEventListener("mousemove",n=>{if(!r||!c)return;const l=r.getBoundingClientRect(),y=n.clientX-l.left,S=n.clientY-l.top;c.style.background=`radial-gradient(circle at ${y}px ${S}px, var(--glow) 0%, transparent 50%)`,c.style.opacity="1"}),r==null||r.addEventListener("mouseleave",()=>{c&&(c.style.opacity="0")});const f=()=>{u=!u,u?(a.textContent="Create Account",h.textContent="Have an account?",o.textContent="Sign in"):(a.textContent="Sign In",h.textContent="New user?",o.textContent="Create account"),d.classList.remove("show")},v=async()=>{const n=t.value.trim(),l=s.value;if(!n){t.classList.add("shake"),d.textContent="Enter username",d.classList.add("show"),setTimeout(()=>t.classList.remove("shake"),400);return}if(!l){s.classList.add("shake"),d.textContent="Enter password",d.classList.add("show"),setTimeout(()=>s.classList.remove("shake"),400);return}i.classList.add("loading"),i.disabled=!0,d.classList.remove("show");try{await e(n,l,u)}catch(y){d.textContent=y instanceof Error?y.message:"Something went wrong",d.classList.add("show"),i.classList.remove("loading"),i.disabled=!1}};i==null||i.addEventListener("click",v),o==null||o.addEventListener("click",f),s==null||s.addEventListener("keydown",n=>{n.key==="Enter"&&v()}),t==null||t.addEventListener("input",()=>{d.classList.remove("show")}),setTimeout(()=>t==null?void 0:t.focus(),100)}}const L={accentColor:"#c9a227",gridSize:"normal"};class p{static load(){const e=localStorage.getItem("site-grid-settings");return e?{...L,...JSON.parse(e)}:{...L}}static save(e){localStorage.setItem("site-grid-settings",JSON.stringify(e)),this.apply(e)}static apply(e){const t=e.accentColor.replace("#",""),s=parseInt(t.slice(0,2),16),i=parseInt(t.slice(2,4),16),a=parseInt(t.slice(4,6),16);document.documentElement.style.setProperty("--accent",e.accentColor),document.documentElement.style.setProperty("--glow",`rgba(${s}, ${i}, ${a}, 0.4)`),document.documentElement.style.setProperty("--glow-warm",`rgba(${s}, ${i}, ${a}, 0.5)`);const o=Math.max(0,s-20),h=Math.max(0,i-30),d=Math.max(0,a-30);document.documentElement.style.setProperty("--accent-warm",`rgb(${o}, ${h}, ${d})`);const r=Math.max(0,s-80),c=Math.min(255,i+30),u=Math.min(255,a+30);document.documentElement.style.setProperty("--accent-sage",`rgb(${r}, ${c}, ${u})`),document.documentElement.style.setProperty("--bg-accent-tint",`rgba(${s}, ${i}, ${a}, 0.06)`),document.documentElement.style.setProperty("--glass-tint",`rgba(${s}, ${i}, ${a}, 0.12)`),document.documentElement.style.setProperty("--border-accent",`rgba(${s}, ${i}, ${a}, 0.4)`),document.documentElement.style.setProperty("--cursor-glow",`rgba(${s}, ${i}, ${a}, 0.4)`)}static render(e){return`
    ...
    `,document.body.appendChild(s),(d=window.lucide)==null||d.createIcons();let i=[];try{i=await(await fetch("https://unpkg.com/lucide@latest/icons.json")).json()}catch{i=["globe","file","image","code","link","star","heart","bookmark"]}const a=document.getElementById("icon-grid"),o=document.getElementById("icon-search"),h=c=>{var f;const u=c?i.filter(v=>v.toLowerCase().includes(c.toLowerCase())):i.slice(0,200);a.innerHTML=u.slice(0,100).map(v=>`<button class="icon-option ${t.icon===v?"selected":""}" data-icon="${v}">
          <i data-lucide="${v}"></i>
        </button>`).join(""),(f=window.lucide)==null||f.createIcons()};h(""),a.addEventListener("click",async c=>{const u=c.target.closest(".icon-option");if(!u)return;const f=u.dataset.icon;try{await this.apiRequest(`${g}/sites/${e}`,{method:"PUT",body:JSON.stringify({icon:f})}),await this.loadSites(),this.render(),s.remove()}catch{alert("Icon update failed")}}),o.addEventListener("input",()=>h(o.value)),(r=document.getElementById("settings-close"))==null||r.addEventListener("click",()=>s.remove()),s.addEventListener("click",c=>{c.target===s&&s.remove()})}async handleView(e){if(this.session)try{const t=await this.apiRequest(`${g}/sites/${e}`);this.viewer=new x(t,()=>{this.viewer=null,this.render()}),this.viewer.open()}catch(t){console.error(t)}}}document.addEventListener("DOMContentLoaded",()=>new I);
