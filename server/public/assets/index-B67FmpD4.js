var f=Object.defineProperty;var b=(l,t,e)=>t in l?f(l,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):l[t]=e;var h=(l,t,e)=>b(l,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const a of n.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&i(a)}).observe(document,{childList:!0,subtree:!0});function e(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function i(s){if(s.ep)return;s.ep=!0;const n=e(s);fetch(s.href,n)}})();class k{static render(){return`
    ...
    `}getFullUrl(){return`/sites/${this.site.id}`}escapeHtml(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}}class S{static render(t,e,i){return`
      <header class="header">
    ...
    `}}class m{static render(){return`
      ...
      <div class="login-form-card" id="login-card">
        ...
        <div class="form-inner">
          ...
          <div class="quick-row">
            <span>Quick:</span>
            <button class="quick-chip" data-role="guest">Guest</button>
            <button class="quick-chip" data-role="admin">Admin</button>
          </div>
        </div>
      </div>
      
      <div class="login-error" id="login-error"></div>
    `}static init(t){const e=document.getElementById("login-password"),i=document.getElementById("btn-login"),s=document.getElementById("login-error"),n=document.getElementById("login-card"),a=document.getElementById("card-glow"),d=document.querySelectorAll(".quick-chip");n==null||n.addEventListener("mousemove",r=>{if(!n||!a)return;const c=n.getBoundingClientRect(),g=r.clientX-c.left,w=r.clientY-c.top;a.style.background=`radial-gradient(circle at ${g}px ${w}px, var(--glow) 0%, transparent 50%)`,a.style.opacity="1"}),n==null||n.addEventListener("mouseleave",()=>{a&&(a.style.opacity="0")});const o=async r=>{if(!r.trim()){e.classList.add("shake"),s.textContent="Enter password",s.classList.add("show"),setTimeout(()=>e.classList.remove("shake"),400);return}i.classList.add("loading"),s.classList.remove("show"),await t(r),i.classList.remove("loading")};i==null||i.addEventListener("click",()=>o(e.value)),e==null||e.addEventListener("keydown",r=>{r.key==="Enter"&&o(e.value),s.classList.remove("show")}),d.forEach(r=>{r.addEventListener("click",()=>{o(r.dataset.role)})})}}const y={accentColor:"#c9a227"};class v{static load(){const t=localStorage.getItem("site-grid-settings");return t?{...y,...JSON.parse(t)}:{...y}}static save(t){localStorage.setItem("site-grid-settings",JSON.stringify(t)),this.apply(t)}static apply(t){const e=t.accentColor.replace("#",""),i=parseInt(e.slice(0,2),16),s=parseInt(e.slice(2,4),16),n=parseInt(e.slice(4,6),16);document.documentElement.style.setProperty("--accent",t.accentColor),document.documentElement.style.setProperty("--glow",`rgba(${i}, ${s}, ${n}, 0.4)`);const a=Math.min(255,i+30),d=Math.max(0,s-50),o=Math.max(0,n-30);document.documentElement.style.setProperty("--accent-warm",`rgb(${a}, ${d}, ${o})`);const r=Math.max(0,i-100),c=Math.min(255,s+40),g=Math.min(255,n+20);document.documentElement.style.setProperty("--accent-sage",`rgb(${r}, ${c}, ${g})`),document.documentElement.style.setProperty("--bg-accent-tint",`rgba(${i}, ${s}, ${n}, 0.05)`),document.documentElement.style.setProperty("--glass-tint",`rgba(${i}, ${s}, ${n}, 0.08)`),document.documentElement.style.setProperty("--border-accent",`rgba(${i}, ${s}, ${n}, 0.3)`)}static render(t){return`
    ...
    `,this.attachEventListeners()}attachEventListeners(){const t=document.getElementById("app"),e=t.querySelector("#logout-btn");e==null||e.addEventListener("click",()=>this.handleLogout());const i=t.querySelector("#settings-btn");i==null||i.addEventListener("click",()=>this.showSettings());const s=t.querySelector(".search-input");s==null||s.addEventListener("input",o=>{this.searchQuery=o.target.value,this.filterSites(),this.updateGrid()}),t.querySelectorAll(".delete-btn").forEach(o=>{o.addEventListener("click",async r=>{var g;if(r.stopPropagation(),((g=this.session)==null?void 0:g.role)!=="admin")return;const c=o.dataset.id;await this.handleDelete(c)})}),t.querySelectorAll(".view-btn").forEach(o=>{o.addEventListener("click",async r=>{r.stopPropagation();const c=o.dataset.id;await this.handleView(c)})}),t.querySelectorAll(".site-card").forEach(o=>{o.addEventListener("click",async()=>{const r=o.dataset.id;await this.handleView(r)})});const n=t.querySelector("#file-input");n==null||n.addEventListener("change",async o=>{var c;const r=(c=o.target.files)==null?void 0:c[0];r&&(await this.handleUpload(r),n.value="")});const a=t.querySelector(".uploader");a==null||a.addEventListener("dragover",o=>{o.preventDefault(),a.classList.add("drag-over")}),a==null||a.addEventListener("dragleave",()=>{a.classList.remove("drag-over")}),a==null||a.addEventListener("drop",async o=>{var c;o.preventDefault(),a.classList.remove("drag-over");const r=(c=o.dataTransfer)==null?void 0:c.files[0];r&&await this.handleUpload(r)});const d=t.querySelector("#back-btn");d==null||d.addEventListener("click",()=>{this.viewer=null,this.render()})}updateGrid(){var e;const t=document.querySelector(".grid-section");t&&(t.innerHTML=p.render(this.filteredSites,((e=this.session)==null?void 0:e.role)==="admin"),this.attachEventListeners())}async handleUpload(t){if(this.session)try{const e=await t.text(),i=t.name.replace(/\.(html|htm)$/i,"");await fetch(`${u}/sites`,{method:"POST",headers:{"Content-Type":"application/json","X-Auth-Token":this.session.token},body:JSON.stringify({name:i,content:e})});const s=await fetch(`${u}/sites`,{headers:{"X-Auth-Token":this.session.token}});this.sites=await s.json(),this.filterSites(),this.render()}catch(e){console.error("Upload failed:",e),alert("Upload failed")}}async handleDelete(t){if(!(!this.session||this.session.role!=="admin")&&confirm("Delete this site?"))try{await fetch(`${u}/sites/${t}`,{method:"DELETE",headers:{"X-Auth-Token":this.session.token}});const e=await fetch(`${u}/sites`,{headers:{"X-Auth-Token":this.session.token}});this.sites=await e.json(),this.filterSites(),this.render()}catch(e){console.error("Delete failed:",e)}}async handleView(t){if(this.session)try{const i=await(await fetch(`${u}/sites/${t}`,{headers:{"X-Auth-Token":this.session.token}})).json();this.viewer=new L(i,()=>{this.viewer=null,this.render()}),this.viewer.open()}catch(e){console.error("Load failed:",e)}}}document.addEventListener("DOMContentLoaded",()=>{new E});
