var $=Object.defineProperty;var k=(m,e,t)=>e in m?$(m,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):m[e]=t;var y=(m,e,t)=>k(m,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const n of a.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&i(n)}).observe(document,{childList:!0,subtree:!0});function t(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function i(s){if(s.ep)return;s.ep=!0;const a=t(s);fetch(s.href,a)}})();class C{static render(){return`
      <div class="uploader" id="uploader">
        <div class="uploader-icon">
          <i data-lucide="upload-cloud"></i>
        </div>
        <h2>HTML-Dateien hochladen</h2>
        <p>Ziehe Dateien hierher oder klicke zum Auswaehlen</p>
        <input type="file" id="file-input" accept=".html,.htm" class="file-input" multiple>
        <label for="file-input" class="upload-btn">
          <i data-lucide="file-up"></i>
          Dateien auswaehlen
        </label>
      </div>
    `}static init(){window.lucide&&lucide.createIcons()}}class b{static render(e,t=!1,i=[]){return e.length===0?`
            ...
            <div class="site-card" data-id="${s.id}" draggable="true">
              <div class="card-preview">
                ...
                ${s.tags&&s.tags.length?`
                  <div class="card-tags">
                    ${s.tags.map(n=>`<span class="card-tag" style="background:${n.color}">${b.escapeHtml(n.name)}</span>`).join("")}
                  </div>
                `:""}
              </div>
              <div class="card-info">
    ...
    `}}class T{static render(e,t,i,s,a,n,g,d,o){return`
      <header class="header">
    ...
    `}static init(){window.lucide&&lucide.createIcons()}}class q{static render(){return`
    ...
    `}static init(e){window.lucide&&lucide.createIcons();const t=document.getElementById("login-username"),i=document.getElementById("login-password"),s=document.getElementById("btn-login"),a=s.querySelector(".btn-text"),n=document.getElementById("btn-toggle"),g=document.getElementById("toggle-text"),d=document.getElementById("login-error"),o=document.getElementById("login-card"),l=document.getElementById("card-glow");let h=!1;o==null||o.addEventListener("mousemove",r=>{if(!o||!l)return;const c=o.getBoundingClientRect(),p=r.clientX-c.left,S=r.clientY-c.top;l.style.background=`radial-gradient(circle at ${p}px ${S}px, var(--glow) 0%, transparent 50%)`,l.style.opacity="1"}),o==null||o.addEventListener("mouseleave",()=>{l&&(l.style.opacity="0")});const w=()=>{h=!h,h?(a.textContent="Create Account",g.textContent="Have an account?",n.textContent="Sign in"):(a.textContent="Sign In",g.textContent="New user?",n.textContent="Create account"),d.classList.remove("show")},f=async()=>{const r=t.value.trim(),c=i.value;if(!r){t.classList.add("shake"),d.textContent="Enter username",d.classList.add("show"),setTimeout(()=>t.classList.remove("shake"),400);return}if(!c){i.classList.add("shake"),d.textContent="Enter password",d.classList.add("show"),setTimeout(()=>i.classList.remove("shake"),400);return}s.classList.add("loading"),s.disabled=!0,d.classList.remove("show");try{await e(r,c,h)}catch(p){d.textContent=p instanceof Error?p.message:"Something went wrong",d.classList.add("show"),s.classList.remove("loading"),s.disabled=!1}};s==null||s.addEventListener("click",f),n==null||n.addEventListener("click",w),i==null||i.addEventListener("keydown",r=>{r.key==="Enter"&&f()}),t==null||t.addEventListener("input",()=>{d.classList.remove("show")}),setTimeout(()=>t==null?void 0:t.focus(),100)}}const L={accentColor:"#c9a227",gridSize:"normal"};class v{static load(){const e=localStorage.getItem("site-grid-settings");return e?{...L,...JSON.parse(e)}:{...L}}static save(e){localStorage.setItem("site-grid-settings",JSON.stringify(e)),this.apply(e)}static apply(e){const t=e.accentColor.replace("#",""),i=parseInt(t.slice(0,2),16),s=parseInt(t.slice(2,4),16),a=parseInt(t.slice(4,6),16);document.documentElement.style.setProperty("--accent",e.accentColor),document.documentElement.style.setProperty("--glow",`rgba(${i}, ${s}, ${a}, 0.4)`),document.documentElement.style.setProperty("--glow-warm",`rgba(${i}, ${s}, ${a}, 0.5)`);const n=Math.max(0,i-20),g=Math.max(0,s-30),d=Math.max(0,a-30);document.documentElement.style.setProperty("--accent-warm",`rgb(${n}, ${g}, ${d})`);const o=Math.max(0,i-80),l=Math.min(255,s+30),h=Math.min(255,a+30);document.documentElement.style.setProperty("--accent-sage",`rgb(${o}, ${l}, ${h})`),document.documentElement.style.setProperty("--bg-accent-tint",`rgba(${i}, ${s}, ${a}, 0.06)`),document.documentElement.style.setProperty("--glass-tint",`rgba(${i}, ${s}, ${a}, 0.12)`),document.documentElement.style.setProperty("--border-accent",`rgba(${i}, ${s}, ${a}, 0.4)`),document.documentElement.style.setProperty("--cursor-glow",`rgba(${i}, ${s}, ${a}, 0.4)`)}static render(e){return`
