var $=Object.defineProperty;var k=(v,e,t)=>e in v?$(v,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):v[e]=t;var b=(v,e,t)=>k(v,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function t(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(i){if(i.ep)return;i.ep=!0;const a=t(i);fetch(i.href,a)}})();class C{static render(){return`
            ...
            <div class="site-card" data-id="${i.id}" draggable="true">
              <div class="card-preview">
                ...
                ${i.tags&&i.tags.length?`
                  <div class="card-tags">
                    ${i.tags.map(o=>`<span class="card-tag" style="background:${o.color}">${w.escapeHtml(o.name)}</span>`).join("")}
                  </div>
                `:""}
              </div>
              <div class="card-info">
    ...
    `}static escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}}class x{constructor(e,t){b(this,"site");b(this,"onClose");this.site=e,this.onClose=t}open(){var a;const e=document.querySelector(".app-container");if(!e)return;const t=document.createElement("div");t.className="viewer-overlay",t.innerHTML=this.render(),e.appendChild(t),window.lucide&&window.lucide.createIcons();const s=document.getElementById("site-iframe"),i=document.getElementById("iframe-loader");if(s&&this.site.content){s.onload=()=>{i==null||i.classList.add("hidden"),s.classList.add("loaded");try{const r=s.contentDocument;r&&(r.documentElement.style.backgroundColor="#0a0a0f",r.body.style.backgroundColor="#0a0a0f")}catch{}};const o=getComputedStyle(document.documentElement).getPropertyValue("--bg-deep")||"#0a0a0f",h=getComputedStyle(document.documentElement).getPropertyValue("--accent")||"#c9a227",d=getComputedStyle(document.documentElement).getPropertyValue("--border-accent")||"#333";s.srcdoc=`<html><body style="background:${o};margin:0;display:flex;align-items:center;justify-content:center;height:100vh;">
      ...
      </body></html>`,setTimeout(()=>{s.srcdoc=this.site.content},100)}(a=document.getElementById("viewer-close"))==null||a.addEventListener("click",()=>{t.remove(),this.onClose()}),t.addEventListener("click",o=>{o.target===t&&(t.remove(),this.onClose())}),document.addEventListener("keydown",o=>{o.key==="Escape"&&(t.remove(),this.onClose())})}render(){return`
    ...
    `}}class T{static render(e,t,s,i,a,o,h,d,r){return`
      <header class="header">
    ...
    `}static init(e){const t=document.getElementById("login-username"),s=document.getElementById("login-password"),i=document.getElementById("btn-login"),a=i.querySelector(".btn-text"),o=document.getElementById("btn-toggle"),h=document.getElementById("toggle-text"),d=document.getElementById("login-error"),r=document.getElementById("login-card"),l=document.getElementById("card-glow");let u=!1;r==null||r.addEventListener("mousemove",n=>{if(!r||!l)return;const c=r.getBoundingClientRect(),y=n.clientX-c.left,S=n.clientY-c.top;l.style.background=`radial-gradient(circle at ${y}px ${S}px, var(--glow) 0%, transparent 50%)`,l.style.opacity="1"}),r==null||r.addEventListener("mouseleave",()=>{l&&(l.style.opacity="0")});const m=()=>{u=!u,u?(a.textContent="Create Account",h.textContent="Have an account?",o.textContent="Sign in"):(a.textContent="Sign In",h.textContent="New user?",o.textContent="Create account"),d.classList.remove("show")},f=async()=>{const n=t.value.trim(),c=s.value;if(!n){t.classList.add("shake"),d.textContent="Enter username",d.classList.add("show"),setTimeout(()=>t.classList.remove("shake"),400);return}if(!c){s.classList.add("shake"),d.textContent="Enter password",d.classList.add("show"),setTimeout(()=>s.classList.remove("shake"),400);return}i.classList.add("loading"),i.disabled=!0,d.classList.remove("show");try{await e(n,c,u)}catch(y){d.textContent=y instanceof Error?y.message:"Something went wrong",d.classList.add("show"),i.classList.remove("loading"),i.disabled=!1}};i==null||i.addEventListener("click",f),o==null||o.addEventListener("click",m),s==null||s.addEventListener("keydown",n=>{n.key==="Enter"&&f()}),t==null||t.addEventListener("input",()=>{d.classList.remove("show")}),setTimeout(()=>t==null?void 0:t.focus(),100)}}const L={accentColor:"#c9a227",gridSize:"normal"};class p{static load(){const e=localStorage.getItem("site-grid-settings");return e?{...L,...JSON.parse(e)}:{...L}}static save(e){localStorage.setItem("site-grid-settings",JSON.stringify(e)),this.apply(e)}static apply(e){const t=e.accentColor.replace("#",""),s=parseInt(t.slice(0,2),16),i=parseInt(t.slice(2,4),16),a=parseInt(t.slice(4,6),16);document.documentElement.style.setProperty("--accent",e.accentColor),document.documentElement.style.setProperty("--glow",`rgba(${s}, ${i}, ${a}, 0.4)`),document.documentElement.style.setProperty("--glow-warm",`rgba(${s}, ${i}, ${a}, 0.5)`);const o=Math.max(0,s-20),h=Math.max(0,i-30),d=Math.max(0,a-30);document.documentElement.style.setProperty("--accent-warm",`rgb(${o}, ${h}, ${d})`);const r=Math.max(0,s-80),l=Math.min(255,i+30),u=Math.min(255,a+30);document.documentElement.style.setProperty("--accent-sage",`rgb(${r}, ${l}, ${u})`),document.documentElement.style.setProperty("--bg-accent-tint",`rgba(${s}, ${i}, ${a}, 0.06)`),document.documentElement.style.setProperty("--glass-tint",`rgba(${s}, ${i}, ${a}, 0.12)`),document.documentElement.style.setProperty("--border-accent",`rgba(${s}, ${i}, ${a}, 0.4)`),document.documentElement.style.setProperty("--cursor-glow",`rgba(${s}, ${i}, ${a}, 0.4)`)}static render(e){return`
    ...
    `,document.body.appendChild(s);let i=[];try{const u=(await(await fetch("https://cdn.jsdelivr.net/npm/lucide@latest/dist/esm/lucide.js")).text()).matchAll(/export \{ default as ([A-Z][a-zA-Z]+)/g);for(const m of u){const n=m[1].replace(/([A-Z])/g,"-$1").toLowerCase().replace(/^-/,"");i.push(n)}}catch{i=["globe","file","image","code","link","star","heart"]}const a=document.getElementById("icon-grid"),o=document.getElementById("icon-search"),h=r=>{var u;const l=r?i.filter(m=>m.toLowerCase().includes(r.toLowerCase())):i.slice(0,200);a.innerHTML=l.slice(0,80).map(m=>`<button class="icon-option ${t.icon===m?"selected":""}" data-icon="${m}" title="${m}">
          <i data-lucide="${m}"></i>
        </button>`).join(""),(u=window.lucide)==null||u.createIcons()};h(""),a.addEventListener("click",async r=>{const l=r.target.closest(".icon-option");if(!l)return;const u=l.dataset.icon;try{await this.apiRequest(`${g}/sites/${e}`,{method:"PUT",body:JSON.stringify({icon:u})}),await this.loadSites(),this.render(),s.remove()}catch{alert("Icon update failed")}}),o.addEventListener("input",()=>h(o.value)),(d=document.getElementById("settings-close"))==null||d.addEventListener("click",()=>s.remove()),s.addEventListener("click",r=>{r.target===s&&s.remove()})}async handleView(e){if(this.session)try{const t=await this.apiRequest(`${g}/sites/${e}`);this.viewer=new x(t,()=>{this.viewer=null,this.render()}),this.viewer.open()}catch(t){console.error(t)}}}document.addEventListener("DOMContentLoaded",()=>new I);
