var S=Object.defineProperty;var x=(c,e,t)=>e in c?S(c,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):c[e]=t;var g=(c,e,t)=>x(c,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const d of n.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function t(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function i(s){if(s.ep)return;s.ep=!0;const n=t(s);fetch(s.href,n)}})();class ${static render(){return`
    ...
    `}}class C{static render(e,t,i,s,n,d){return`
      <header class="header">
    ...
    `}static init(e){const t=document.getElementById("login-username"),i=document.getElementById("login-password"),s=document.getElementById("btn-login"),n=document.getElementById("btn-toggle"),d=document.getElementById("toggle-text"),o=document.getElementById("login-error"),a=document.getElementById("login-card"),l=document.getElementById("card-glow");let r=!1;a==null||a.addEventListener("mousemove",m=>{if(!a||!l)return;const p=a.getBoundingClientRect(),f=m.clientX-p.left,E=m.clientY-p.top;l.style.background=`radial-gradient(circle at ${f}px ${E}px, var(--glow) 0%, transparent 50%)`,l.style.opacity="1"}),a==null||a.addEventListener("mouseleave",()=>{l&&(l.style.opacity="0")});const h=()=>{r=!r,r?(s.querySelector("span").textContent="Create Account",d.textContent="Have an account?",n.textContent="Sign in"):(s.querySelector("span").textContent="Sign In",d.textContent="New user?",n.textContent="Create account"),o.classList.remove("show")},u=async()=>{const m=t.value.trim(),p=i.value;if(!m){t.classList.add("shake"),o.textContent="Enter username",o.classList.add("show"),setTimeout(()=>t.classList.remove("shake"),400);return}if(!p){i.classList.add("shake"),o.textContent="Enter password",o.classList.add("show"),setTimeout(()=>i.classList.remove("shake"),400);return}s.classList.add("loading"),o.classList.remove("show");try{await e(m,p,r)}catch(f){o.textContent=f instanceof Error?f.message:"Something went wrong",o.classList.add("show")}s.classList.remove("loading")};s==null||s.addEventListener("click",u),n==null||n.addEventListener("click",h),i==null||i.addEventListener("keydown",m=>{m.key==="Enter"&&u()}),t==null||t.addEventListener("input",()=>{o.classList.remove("show")}),setTimeout(()=>t==null?void 0:t.focus(),100)}}const L={accentColor:"#c9a227"};class y{static load(){const e=localStorage.getItem("site-grid-settings");return e?{...L,...JSON.parse(e)}:{...L}}static save(e){localStorage.setItem("site-grid-settings",JSON.stringify(e)),this.apply(e)}static apply(e){const t=e.accentColor.replace("#",""),i=parseInt(t.slice(0,2),16),s=parseInt(t.slice(2,4),16),n=parseInt(t.slice(4,6),16);document.documentElement.style.setProperty("--accent",e.accentColor),document.documentElement.style.setProperty("--glow",`rgba(${i}, ${s}, ${n}, 0.4)`);const d=Math.min(255,i+30),o=Math.max(0,s-50),a=Math.max(0,n-30);document.documentElement.style.setProperty("--accent-warm",`rgb(${d}, ${o}, ${a})`);const l=Math.max(0,i-100),r=Math.min(255,s+40),h=Math.min(255,n+20);document.documentElement.style.setProperty("--accent-sage",`rgb(${l}, ${r}, ${h})`),document.documentElement.style.setProperty("--bg-accent-tint",`rgba(${i}, ${s}, ${n}, 0.05)`),document.documentElement.style.setProperty("--glass-tint",`rgba(${i}, ${s}, ${n}, 0.08)`),document.documentElement.style.setProperty("--border-accent",`rgba(${i}, ${s}, ${n}, 0.3)`)}static render(e){return`
    ...
    `);(i=document.getElementById("settings-close"))==null||i.addEventListener("click",()=>e.remove()),this.loadUserList()}async loadUserList(){const e=document.getElementById("user-list");if(!(!e||!this.session))try{const t=await this.apiRequest(`${v}/admin/users`);e.innerHTML=t.map(i=>`
      ...
      `).join(""),e.querySelectorAll(".promote").forEach(i=>{i.addEventListener("click",()=>void this.promoteUser(i.dataset.id))}),e.querySelectorAll(".delete").forEach(i=>{i.addEventListener("click",()=>void this.deleteUser(i.dataset.id))})}catch{e.innerHTML='<div class="error">Failed to load</div>'}}async promoteUser(e){if(confirm("Promote this user to admin?"))try{await this.apiRequest(`${v}/admin/users/${e}`,{method:"PUT",body:JSON.stringify({role:"admin"})}),this.loadUserList()}catch(t){alert(t instanceof Error?t.message:"Failed to promote")}}async deleteUser(e){if(confirm("Delete this user?"))try{await this.apiRequest(`${v}/admin/users/${e}`,{method:"DELETE"}),this.loadUserList()}catch(t){alert(t instanceof Error?t.message:"Failed to delete")}}escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}renderLogin(){const e=document.getElementById("app");e&&(e.innerHTML=b.render(),b.init(this.handleAuth.bind(this)))}render(){var t,i,s,n;const e=document.getElementById("app");e&&(e.innerHTML=`
    ...
    `,this.attachEventListeners(),this.loadSidebarData())}attachEventListeners(){var n,d,o,a,l;const e=document.getElementById("app");if(!e)return;(n=e.querySelector("#logout-btn"))==null||n.addEventListener("click",()=>this.handleLogout()),(d=e.querySelector("#settings-btn"))==null||d.addEventListener("click",()=>this.showSettings()),(o=e.querySelector("#leaderboard-btn"))==null||o.addEventListener("click",()=>this.showLeaderboard()),(a=e.querySelector("#users-btn"))==null||a.addEventListener("click",()=>this.showUserManagement());const t=e.querySelector(".search-input");t==null||t.addEventListener("input",r=>{this.searchQuery=r.target.value,this.filterSites(),this.updateGrid()}),e.querySelectorAll(".delete-btn").forEach(r=>{r.addEventListener("click",h=>{var u;h.stopPropagation(),((u=this.session)==null?void 0:u.role)==="admin"&&this.handleDelete(r.dataset.id)})}),e.querySelectorAll(".view-btn, .site-card").forEach(r=>{r.addEventListener("click",()=>{this.handleView(r.dataset.id)})});const i=e.querySelector("#file-input");i==null||i.addEventListener("change",async r=>{var u;const h=(u=r.target.files)==null?void 0:u[0];h&&(await this.handleUpload(h),i.value="")});const s=e.querySelector(".uploader");s==null||s.addEventListener("dragover",r=>{r.preventDefault(),s.classList.add("drag-over")}),s==null||s.addEventListener("dragleave",()=>s.classList.remove("drag-over")),s==null||s.addEventListener("drop",async r=>{var u;r.preventDefault(),s.classList.remove("drag-over");const h=(u=r.dataTransfer)==null?void 0:u.files[0];h&&await this.handleUpload(h)}),(l=e.querySelector("#back-btn"))==null||l.addEventListener("click",()=>{this.viewer=null,this.render()})}updateGrid(){var t;const e=document.querySelector(".grid-section");e&&(e.innerHTML=w.render(this.filteredSites,((t=this.session)==null?void 0:t.role)==="admin"),this.attachEventListeners())}async handleAuth(e,t,i){const s=i?"/auth/register":"/auth/login";try{const n=await this.apiRequest(`${v}${s}`,{method:"POST",body:JSON.stringify({username:e,password:t})});this.session={token:n.token,role:n.role,username:n.username,userId:n.id},this.saveSession(),await this.loadSites(),this.render()}catch(n){throw n}}async handleLogout(){this.clearSession(),this.sites=[],this.filteredSites=[],this.viewer=null,this.renderLogin()}async handleUpload(e){if(this.session)try{const t=await e.text();await this.apiRequest(`${v}/sites`,{method:"POST",body:JSON.stringify({name:e.name.replace(/\.(html?)$/i,""),content:t})}),await this.loadSites(),this.render()}catch(t){console.error("Upload failed:",t),alert("Upload failed")}}async handleDelete(e){if(confirm("Delete this site?"))try{await this.apiRequest(`${v}/sites/${e}`,{method:"DELETE"}),this.sites=this.sites.filter(t=>t.id!==e),this.filterSites(),this.render()}catch(t){console.error("Delete failed:",t)}}async handleView(e){if(this.session)try{const t=await this.apiRequest(`${v}/sites/${e}`);this.viewer=new k(t,()=>{this.viewer=null,this.render()}),this.viewer.open()}catch(t){console.error("Load failed:",t)}}}document.addEventListener("DOMContentLoaded",()=>new B);
