var $=Object.defineProperty;var k=(v,e,t)=>e in v?$(v,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):v[e]=t;var b=(v,e,t)=>k(v,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function t(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(i){if(i.ep)return;i.ep=!0;const a=t(i);fetch(i.href,a)}})();class C{static render(){return`
    ...
    `,document.body.appendChild(s);let i=[];try{const u=(await(await fetch("https://cdn.jsdelivr.net/npm/lucide@latest/dist/esm/lucide.js")).text()).matchAll(/export \{ default as ([A-Z][a-zA-Z]+)/g);for(const m of u){const n=m[1].replace(/([A-Z])/g,"-$1").toLowerCase().replace(/^-/,"");i.push(n)}}catch{i=["globe","file","image","code","link","star","heart"]}const a=document.getElementById("icon-grid"),r=document.getElementById("icon-search"),h=o=>{var u;const l=o?i.filter(m=>m.toLowerCase().includes(o.toLowerCase())):i.slice(0,200);a.innerHTML=l.slice(0,80).map(m=>`<button class="icon-option ${t.icon===m?"selected":""}" data-icon="${m}" title="${m}">
          <i data-lucide="${m}"></i>
        </button>`).join(""),(u=window.lucide)==null||u.createIcons()};h(""),a.addEventListener("click",async o=>{const l=o.target.closest(".icon-option");if(!l)return;const u=l.dataset.icon;try{await this.apiRequest(`${g}/sites/${e}`,{method:"PUT",body:JSON.stringify({icon:u})}),await this.loadSites(),this.render(),s.remove()}catch{alert("Icon update failed")}}),r.addEventListener("input",()=>h(r.value)),(d=document.getElementById("settings-close"))==null||d.addEventListener("click",()=>s.remove()),s.addEventListener("click",o=>{o.target===s&&s.remove()})}async handleView(e){if(this.session)try{const t=await this.apiRequest(`${g}/sites/${e}`);this.viewer=new x(t,()=>{this.viewer=null,this.render()}),this.viewer.open()}catch(t){console.error(t)}}}document.addEventListener("DOMContentLoaded",()=>new I);
