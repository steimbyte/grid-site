var x=Object.defineProperty;var S=(c,e,t)=>e in c?x(c,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):c[e]=t;var g=(c,e,t)=>S(c,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const d of n.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&i(d)}).observe(document,{childList:!0,subtree:!0});function t(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function i(s){if(s.ep)return;s.ep=!0;const n=t(s);fetch(s.href,n)}})();class k{static render(){return`
    ...
    `);(t=document.getElementById("settings-close"))==null||t.addEventListener("click",()=>e.remove()),this.loadLeaderboard()}async loadLeaderboard(){const e=document.getElementById("leaderboard-list");if(e)try{const t=await fetch(`${u}/leaderboard`).then(i=>i.json());if(!t.length){e.innerHTML='<div class="leaderboard-empty">No visits yet</div>';return}e.innerHTML=t.map((i,s)=>`
      ...
      `).join("")}catch{e.innerHTML='<div class="leaderboard-empty">Failed to load</div>'}}async loadSidebarData(){try{const t=await fetch(`${u}/stats`).then(d=>d.json()),i=document.getElementById("stat-logins"),s=document.getElementById("stat-users"),n=document.getElementById("stat-sites");i&&(i.textContent=String(t.totalLogins)),s&&(s.textContent=String(t.totalUsers)),n&&(n.textContent=String(t.totalSites))}catch{}const e=document.getElementById("sidebar-leaderboard");if(e)try{const t=await fetch(`${u}/leaderboard?limit=5`).then(i=>i.json());if(!t.length){e.innerHTML='<div class="empty">No visits yet</div>';return}e.innerHTML=t.map((i,s)=>`
    ...
    `);(i=document.getElementById("settings-close"))==null||i.addEventListener("click",()=>e.remove()),this.loadUserList()}async loadUserList(){const e=document.getElementById("user-list");if(!(!e||!this.session))try{const t=await this.apiRequest(`${u}/admin/users`);e.innerHTML=t.map(i=>`
      ...
      `).join(""),e.querySelectorAll(".promote").forEach(i=>{i.addEventListener("click",()=>void this.promoteUser(i.dataset.id))}),e.querySelectorAll(".delete").forEach(i=>{i.addEventListener("click",()=>void this.deleteUser(i.dataset.id))})}catch{e.innerHTML='<div class="error">Failed to load</div>'}}async promoteUser(e){if(confirm("Promote this user to admin?"))try{await this.apiRequest(`${u}/admin/users/${e}`,{method:"PUT",body:JSON.stringify({role:"admin"})}),this.loadUserList()}catch(t){alert(t instanceof Error?t.message:"Failed to promote")}}async deleteUser(e){if(confirm("Delete this user?"))try{await this.apiRequest(`${u}/admin/users/${e}`,{method:"DELETE"}),this.loadUserList()}catch(t){alert(t instanceof Error?t.message:"Failed to delete")}}escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}renderLogin(){const e=document.getElementById("app");e&&(e.innerHTML=b.render(),b.init(this.handleAuth.bind(this)))}render(){var t,i,s,n;const e=document.getElementById("app");e&&(e.innerHTML=`
