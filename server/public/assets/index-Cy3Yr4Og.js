var S=Object.defineProperty;var $=(v,e,t)=>e in v?S(v,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):v[e]=t;var y=(v,e,t)=>$(v,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const r of n.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function t(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(i){if(i.ep)return;i.ep=!0;const n=t(i);fetch(i.href,n)}})();class k{static render(){return`
        ...
        <div class="site-card" data-id="${n.id}" draggable="true">
          <div class="card-preview">
            ...
            ${n.tags&&n.tags.length?`
              <div class="card-tags">
                ${n.tags.map(g=>`<span class="card-tag" style="background:${g.color}">${w.escapeHtml(g.name)}</span>`).join("")}
              </div>
            `:""}
          </div>
          <div class="card-info">
    ...
    `}}class M{static render(e,t,s,i,n,r,g,a,o){return`
      <header class="header">
    ...
    `}static init(e){const t=document.getElementById("login-username"),s=document.getElementById("login-password"),i=document.getElementById("btn-login"),n=i.querySelector(".btn-text"),r=document.getElementById("btn-toggle"),g=document.getElementById("toggle-text"),a=document.getElementById("login-error"),o=document.getElementById("login-card"),c=document.getElementById("card-glow");let u=!1;o==null||o.addEventListener("mousemove",l=>{if(!o||!c)return;const m=o.getBoundingClientRect(),f=l.clientX-m.left,x=l.clientY-m.top;c.style.background=`radial-gradient(circle at ${f}px ${x}px, var(--glow) 0%, transparent 50%)`,c.style.opacity="1"}),o==null||o.addEventListener("mouseleave",()=>{c&&(c.style.opacity="0")});const b=()=>{u=!u,u?(n.textContent="Create Account",g.textContent="Have an account?",r.textContent="Sign in"):(n.textContent="Sign In",g.textContent="New user?",r.textContent="Create account"),a.classList.remove("show")},d=async()=>{const l=t.value.trim(),m=s.value;if(!l){t.classList.add("shake"),a.textContent="Enter username",a.classList.add("show"),setTimeout(()=>t.classList.remove("shake"),400);return}if(!m){s.classList.add("shake"),a.textContent="Enter password",a.classList.add("show"),setTimeout(()=>s.classList.remove("shake"),400);return}i.classList.add("loading"),i.disabled=!0,a.classList.remove("show");try{await e(l,m,u)}catch(f){a.textContent=f instanceof Error?f.message:"Something went wrong",a.classList.add("show"),i.classList.remove("loading"),i.disabled=!1}};i==null||i.addEventListener("click",d),r==null||r.addEventListener("click",b),s==null||s.addEventListener("keydown",l=>{l.key==="Enter"&&d()}),t==null||t.addEventListener("input",()=>{a.classList.remove("show")}),setTimeout(()=>t==null?void 0:t.focus(),100)}}const L={accentColor:"#c9a227",gridSize:"normal"};class p{static load(){const e=localStorage.getItem("site-grid-settings");return e?{...L,...JSON.parse(e)}:{...L}}static save(e){localStorage.setItem("site-grid-settings",JSON.stringify(e)),this.apply(e)}static apply(e){const t=e.accentColor.replace("#",""),s=parseInt(t.slice(0,2),16),i=parseInt(t.slice(2,4),16),n=parseInt(t.slice(4,6),16);document.documentElement.style.setProperty("--accent",e.accentColor),document.documentElement.style.setProperty("--glow",`rgba(${s}, ${i}, ${n}, 0.4)`),document.documentElement.style.setProperty("--glow-warm",`rgba(${s}, ${i}, ${n}, 0.5)`);const r=Math.max(0,s-20),g=Math.max(0,i-30),a=Math.max(0,n-30);document.documentElement.style.setProperty("--accent-warm",`rgb(${r}, ${g}, ${a})`);const o=Math.max(0,s-80),c=Math.min(255,i+30),u=Math.min(255,n+30);document.documentElement.style.setProperty("--accent-sage",`rgb(${o}, ${c}, ${u})`),document.documentElement.style.setProperty("--bg-accent-tint",`rgba(${s}, ${i}, ${n}, 0.06)`),document.documentElement.style.setProperty("--glass-tint",`rgba(${s}, ${i}, ${n}, 0.12)`),document.documentElement.style.setProperty("--border-accent",`rgba(${s}, ${i}, ${n}, 0.4)`),document.documentElement.style.setProperty("--cursor-glow",`rgba(${s}, ${i}, ${n}, 0.4)`)}static render(e){return`
    ...
    `,this.attachEventListeners(),this.loadSidebarData())}attachEventListeners(){var n,r,g,a,o,c,u,b;const e=document.getElementById("app");if(!e)return;(n=e.querySelector("#logout-btn"))==null||n.addEventListener("click",()=>this.handleLogout()),(r=e.querySelector("#settings-btn"))==null||r.addEventListener("click",()=>this.showSettings()),(g=e.querySelector("#leaderboard-btn"))==null||g.addEventListener("click",()=>this.showLeaderboard()),(a=e.querySelector("#grid-size-btn"))==null||a.addEventListener("click",()=>this.toggleGridSize()),(o=e.querySelector("#users-btn"))==null||o.addEventListener("click",()=>this.showUserManagement()),(c=e.querySelector("#tags-btn"))==null||c.addEventListener("click",()=>this.showTags()),(u=e.querySelector("#export-btn"))==null||u.addEventListener("click",()=>this.exportZip());const t=e.querySelector(".search-input");t==null||t.addEventListener("input",d=>{this.searchQuery=d.target.value,this.filterSites(),this.updateGrid()}),e.querySelectorAll(".delete-btn").forEach(d=>d.addEventListener("click",l=>{var m;l.stopPropagation(),((m=this.session)==null?void 0:m.role)==="admin"&&this.handleDelete(d.dataset.id)})),e.querySelectorAll(".view-btn, .site-card").forEach(d=>d.addEventListener("click",()=>void this.handleView(d.dataset.id)));const s=e.querySelector("#file-input");s==null||s.addEventListener("change",async d=>{const l=d.target.files;l!=null&&l.length&&(await this.handleUploadMultiple(Array.from(l)),s.value="")});const i=e.querySelector(".uploader");i==null||i.addEventListener("dragover",d=>{d.preventDefault(),i.classList.add("drag-over")}),i==null||i.addEventListener("dragleave",()=>i.classList.remove("drag-over")),i==null||i.addEventListener("drop",async d=>{var m;d.preventDefault(),i.classList.remove("drag-over");const l=(m=d.dataTransfer)==null?void 0:m.files;l!=null&&l.length&&await this.handleUploadMultiple(Array.from(l))}),(b=e.querySelector("#back-btn"))==null||b.addEventListener("click",()=>{this.viewer=null,this.render()}),document.addEventListener("keydown",d=>{var l;d.target instanceof HTMLInputElement||(d.key==="/"&&(d.preventDefault(),t==null||t.focus()),(d.key==="n"||d.key==="N")&&(d.preventDefault(),(l=document.querySelector("#file-input"))==null||l.click()))})}async loadSidebarData(){try{const t=await fetch(`${h}/stats`).then(s=>s.json());document.getElementById("stat-logins").textContent=String(t.totalLogins),document.getElementById("stat-users").textContent=String(t.totalUsers),document.getElementById("stat-sites").textContent=String(t.totalSites)}catch{}const e=document.getElementById("sidebar-leaderboard");if(e)try{const t=await fetch(`${h}/leaderboard?limit=5`).then(s=>s.json());e.innerHTML=t.length?t.map((s,i)=>`<div class="sidebar-item ${i===0?"gold":i===1?"silver":i===2?"bronze":""}"><span class="sidebar-rank">${i+1}</span><span class="sidebar-name">${s.username}</span><span class="sidebar-visits">${s.visits}</span></div>`).join(""):'<div class="empty">No visits</div>'}catch{e.innerHTML='<div class="empty">Failed</div>'}}updateGrid(){var t;const e=document.querySelector(".grid-section");e&&(e.innerHTML=w.render(this.filteredSites,((t=this.session)==null?void 0:t.role)==="admin",this.tags),this.attachEventListeners())}async handleUpload(e){if(this.session)try{const t=await e.text();await this.apiRequest(`${h}/sites`,{method:"POST",body:JSON.stringify({name:e.name.replace(/\.(html?)$/i,""),content:t})}),await this.loadSites(),this.render()}catch(t){console.error(t),alert("Upload failed")}}async handleUploadMultiple(e){if(!this.session||e.length===0)return;let t=0,s=0;for(const i of e)try{const n=await i.text();await this.apiRequest(`${h}/sites`,{method:"POST",body:JSON.stringify({name:i.name.replace(/\.(html?)$/i,""),content:n})}),t++}catch{s++}await this.loadSites(),this.render(),s>0?alert(`${t} uploaded, ${s} failed`):t>1&&alert(`${t} files uploaded`)}async handleDelete(e){if(confirm("Delete?"))try{await this.apiRequest(`${h}/sites/${e}`,{method:"DELETE"}),this.sites=this.sites.filter(t=>t.id!==e),this.filterSites(),this.render()}catch(t){console.error(t)}}async handleView(e){if(this.session)try{const t=await this.apiRequest(`${h}/sites/${e}`);this.viewer=new C(t,()=>{this.viewer=null,this.render()}),this.viewer.open()}catch(t){console.error(t)}}}document.addEventListener("DOMContentLoaded",()=>new T);
