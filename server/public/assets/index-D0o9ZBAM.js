var x=Object.defineProperty;var S=(c,e,t)=>e in c?x(c,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):c[e]=t;var g=(c,e,t)=>S(c,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const l of r.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&i(l)}).observe(document,{childList:!0,subtree:!0});function t(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(s){if(s.ep)return;s.ep=!0;const r=t(s);fetch(s.href,r)}})();class ${static render(){return`
    ...
    `}}class C{static render(e,t,i,s,r,l){return`
      <header class="header">
    ...
    `}static init(e){const t=document.getElementById("login-username"),i=document.getElementById("login-password"),s=document.getElementById("btn-login"),r=document.getElementById("btn-toggle"),l=document.getElementById("toggle-text"),o=document.getElementById("login-error"),a=document.getElementById("login-card"),d=document.getElementById("card-glow");let n=!1;a==null||a.addEventListener("mousemove",m=>{if(!a||!d)return;const p=a.getBoundingClientRect(),b=m.clientX-p.left,E=m.clientY-p.top;d.style.background=`radial-gradient(circle at ${b}px ${E}px, var(--glow) 0%, transparent 50%)`,d.style.opacity="1"}),a==null||a.addEventListener("mouseleave",()=>{d&&(d.style.opacity="0")});const h=()=>{n=!n,n?(s.querySelector("span").textContent="Create Account",l.textContent="Have an account?",r.textContent="Sign in"):(s.querySelector("span").textContent="Sign In",l.textContent="New user?",r.textContent="Create account"),o.classList.remove("show")},u=async()=>{const m=t.value.trim(),p=i.value;if(!m){t.classList.add("shake"),o.textContent="Enter username",o.classList.add("show"),setTimeout(()=>t.classList.remove("shake"),400);return}if(!p){i.classList.add("shake"),o.textContent="Enter password",o.classList.add("show"),setTimeout(()=>i.classList.remove("shake"),400);return}s.classList.add("loading"),o.classList.remove("show");try{await e(m,p,n)}catch(b){o.textContent=b instanceof Error?b.message:"Something went wrong",o.classList.add("show")}s.classList.remove("loading")};s==null||s.addEventListener("click",u),r==null||r.addEventListener("click",h),i==null||i.addEventListener("keydown",m=>{m.key==="Enter"&&u()}),t==null||t.addEventListener("input",()=>{o.classList.remove("show")}),setTimeout(()=>t==null?void 0:t.focus(),100)}}const L={accentColor:"#c9a227"};class y{static load(){const e=localStorage.getItem("site-grid-settings");return e?{...L,...JSON.parse(e)}:{...L}}static save(e){localStorage.setItem("site-grid-settings",JSON.stringify(e)),this.apply(e)}static apply(e){const t=e.accentColor.replace("#",""),i=parseInt(t.slice(0,2),16),s=parseInt(t.slice(2,4),16),r=parseInt(t.slice(4,6),16);document.documentElement.style.setProperty("--accent",e.accentColor),document.documentElement.style.setProperty("--glow",`rgba(${i}, ${s}, ${r}, 0.4)`);const l=Math.min(255,i+30),o=Math.max(0,s-50),a=Math.max(0,r-30);document.documentElement.style.setProperty("--accent-warm",`rgb(${l}, ${o}, ${a})`);const d=Math.max(0,i-100),n=Math.min(255,s+40),h=Math.min(255,r+20);document.documentElement.style.setProperty("--accent-sage",`rgb(${d}, ${n}, ${h})`),document.documentElement.style.setProperty("--bg-accent-tint",`rgba(${i}, ${s}, ${r}, 0.05)`),document.documentElement.style.setProperty("--glass-tint",`rgba(${i}, ${s}, ${r}, 0.08)`),document.documentElement.style.setProperty("--border-accent",`rgba(${i}, ${s}, ${r}, 0.3)`)}static render(e){return`
    ...
    `);(t=document.getElementById("settings-close"))==null||t.addEventListener("click",()=>e.remove()),this.loadLeaderboard()}async loadLeaderboard(){const e=document.getElementById("leaderboard-list");if(e)try{const t=await fetch(`${v}/leaderboard`).then(i=>i.json());if(!t.length){e.innerHTML='<div class="leaderboard-empty">No visits yet</div>';return}e.innerHTML=t.map((i,s)=>`
      ...
      `).join("")}catch{e.innerHTML='<div class="leaderboard-empty">Failed to load</div>'}}async loadSidebarLeaderboard(){const e=document.getElementById("sidebar-leaderboard");if(e)try{const t=await fetch(`${v}/leaderboard?limit=5`).then(i=>i.json());if(!t.length){e.innerHTML='<div class="empty">No visits yet</div>';return}e.innerHTML=t.map((i,s)=>`
    ...
    `);(i=document.getElementById("settings-close"))==null||i.addEventListener("click",()=>e.remove()),this.loadUserList()}async loadUserList(){const e=document.getElementById("user-list");if(!(!e||!this.session))try{const t=await this.apiRequest(`${v}/admin/users`);e.innerHTML=t.map(i=>`
      ...
      `).join(""),e.querySelectorAll(".promote").forEach(i=>{i.addEventListener("click",()=>void this.promoteUser(i.dataset.id))}),e.querySelectorAll(".delete").forEach(i=>{i.addEventListener("click",()=>void this.deleteUser(i.dataset.id))})}catch{e.innerHTML='<div class="error">Failed to load</div>'}}async promoteUser(e){if(confirm("Promote this user to admin?"))try{await this.apiRequest(`${v}/admin/users/${e}`,{method:"PUT",body:JSON.stringify({role:"admin"})}),this.loadUserList()}catch(t){alert(t instanceof Error?t.message:"Failed to promote")}}async deleteUser(e){if(confirm("Delete this user?"))try{await this.apiRequest(`${v}/admin/users/${e}`,{method:"DELETE"}),this.loadUserList()}catch(t){alert(t instanceof Error?t.message:"Failed to delete")}}escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}renderLogin(){const e=document.getElementById("app");e&&(e.innerHTML=f.render(),f.init(this.handleAuth.bind(this)))}render(){var t,i,s,r;const e=document.getElementById("app");e&&(e.innerHTML=`
      <div class="app-container">
        ...
        <div class="main-content">
          ...
          <aside class="leaderboard-sidebar" id="leaderboard-sidebar">
            <h3>🏆 Top Users</h3>
